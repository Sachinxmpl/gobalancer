...
We hereby declare that the work presented in this report titled \emph{LoadGate: A
...
This is to certify that the project titled \emph{LoadGate: A Concurrent L4/L7 Load
...
without the authors' written permission. The source code of LoadGate is released as open
source and is available at \url{https://github.com/Sachinxmpl/loadgate}.

%======================= ABSTRACT =======================
\chapter*{Abstract}
\addcontentsline{toc}{chapter}{Abstract}
LoadGate is a load balancer implemented in Go that distributes client traffic across a
...
the request path, headers, or cookies, at the cost of parsing every request. LoadGate
...
\end{itemize}
Table~\ref{tab:related} places LoadGate alongside these systems.

\begin{table}[htbp]
\centering
\caption{LoadGate in relation to existing load balancers.}
\label{tab:related}
\begin{tabular}{p{3.5cm}cccc}
\toprule
\textbf{Feature} & \textbf{NGINX} & \textbf{HAProxy} & \textbf{Envoy} & \textbf{LoadGate}\\
\midrule
...
tools but poor \emph{teaching artefacts}: their scale hides the core mechanisms. LoadGate
...
\draw[dashed,->] (hup) -- (cfg);
\end{tikzpicture}
\caption{High-level architecture of LoadGate. Solid arrows are the client-request path;
...
failing. Rather than use the standard library's \code{httputil.ReverseProxy}, LoadGate
...
LoadGate is operated through a command-line interface (\code{run} and \code{check}
...
The system is built with \code{make build} and run as \code{loadgate run -c config.yaml}. The
...
\section{Conclusions}
This project designed, implemented, and evaluated LoadGate, a concurrent L4/L7 load balancer
